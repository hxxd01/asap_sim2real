#ifndef A7E5B74F_506D_4F8D_8B92_475FEDC6911F
#define A7E5B74F_506D_4F8D_8B92_475FEDC6911F
#include "rl_sdk.hpp"

class HomieVelModel : public RobotModel {
...
public:
    HomieVelModel(): memoryInfo(Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault)){
    ...
    void Forward(const ModelParams& params, OnnxTensor& onnx_tensor, Observations& obs){
        ...
        // 输入 2: time_step 
        std::vector<tensor_element_t> est_vel;
        for (int i = 0; i < 3; i++)
        {
            est_vel.push_back(static_cast<tensor_element_t>(obs.est_lin_vel[i]));
        }
        inputValues.push_back(Ort::Value::CreateTensor<tensor_element_t>(this->memoryInfo, 
                                                                    est_vel.data(), 
                                                                    est_vel.size(),
                                                                    onnx_tensor.inputShapes[1].data(),
                                                                    onnx_tensor.inputShapes[1].size()));
        // run inference
    ...
    std::string name() const override { return "HomieVel"; }
};


#endif /* A7E5B74F_506D_4F8D_8B92_475FEDC6911F */
