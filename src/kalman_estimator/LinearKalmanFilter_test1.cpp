...
vector_t KalmanFilterEstimate::Update(const scalar_t& dt, RobotState<double> *state) {
  ...
  this->kalmanMatrix.A_matrix.block(0, 3, 3, 3) = dt * Eigen::Matrix<scalar_t, 3, 3>::Identity();
  this->kalmanMatrix.B_matrix.block(0, 0, 3, 3) = 0.5 * dt * dt * Eigen::Matrix<scalar_t, 3, 3>::Identity();
  this->kalmanMatrix.B_matrix.block(3, 0, 3, 3) = dt * Eigen::Matrix<scalar_t, 3, 3>::Identity();
  this->kalmanMatrix.Q_obs_process_noise_covariance_matrix.block(0, 0, 3, 3) = (dt / 20.f) * Eigen::Matrix<scalar_t, 3, 3>::Identity();
  this->kalmanMatrix.Q_obs_process_noise_covariance_matrix.block(3, 3, 3, 3) = (dt * 9.81f / 20.f) * Eigen::Matrix<scalar_t, 3, 3>::Identity();
  this->kalmanMatrix.Q_obs_process_noise_covariance_matrix.block(6, 6, 12, 12) = dt * Eigen::Matrix<scalar_t, 12, 12>::Identity();
...
//   }
  ...
  q.block(0, 0, 3, 3) = this->kalmanMatrix.Q_obs_process_noise_covariance_matrix.block(0, 0, 3, 3) * this->noise_settings.imuProcessNoisePosition;
  q.block(3, 3, 3, 3) = this->kalmanMatrix.Q_obs_process_noise_covariance_matrix.block(3, 3, 3, 3) * this->noise_settings.imuProcessNoiseVelocity;
  q.block(6, 6, 12, 12) = this->kalmanMatrix.Q_obs_process_noise_covariance_matrix.block(6, 6, 12, 12) * this->noise_settings.footProcessNoisePosition;

  // the covariance of the observation noise
  Eigen::Matrix<scalar_t, 28, 28> r = Eigen::Matrix<scalar_t, 28, 28>::Identity();
  r.block(0, 0, 12, 12) = this->kalmanMatrix.R_state_process_noise_covariance_matrix.block(0, 0, 12, 12) * this->noise_settings.footSensorNoisePosition;
  r.block(12, 12, 12, 12) = this->kalmanMatrix.R_state_process_noise_covariance_matrix.block(12, 12, 12, 12) * this->noise_settings.footSensorNoiseVelocity;
  // const int fn = this->robot_info.numThreeDofContacts;
  const int fn = 4;
  r.block(24, 24, fn, fn) = this->kalmanMatrix.R_state_process_noise_covariance_matrix.block(24, 24, fn, fn) * this->noise_settings.footHeightSensorNoise;
  ...
  for (int i = 0; i < 4; i++) {
    ...
    this->kalmanMatrix.foot_positions.segment(3 * i, 3) = -eePos[i];
    this->kalmanMatrix.foot_positions.segment(3 * i, 3)[2] += this->noise_settings.footRadius;
    this->kalmanMatrix.foot_velocities.segment(3 * i, 3) = -eeVel[i];
  }
  ...
  // a_=F c_=H
  y << this->kalmanMatrix.foot_positions, this->kalmanMatrix.foot_velocities, this->feetHeights;  
  ...
  Eigen::Matrix<scalar_t, 18, 18> at = this->kalmanMatrix.A_matrix.transpose();
  Eigen::Matrix<scalar_t, 18, 18> pm = this->kalmanMatrix.A_matrix * this->kalmanMatrix.P_posteriori_covariance_matrix * at + this->kalmanMatrix.Q_obs_process_noise_covariance_matrix;  // Predicted (a priori) estimate covariance
  Eigen::Matrix<scalar_t, 18, 28> cT = this->kalmanMatrix.C_matrix.transpose();
  Eigen::Matrix<scalar_t, 28, 1> yModel = this->kalmanMatrix.C_matrix * this->kalmanMatrix.xHat;
  Eigen::Matrix<scalar_t, 28, 1> ey = y - yModel;  // Innovation or measurement pre-fit residual
  Eigen::Matrix<scalar_t, 28, 28> s = this->kalmanMatrix.C_matrix * pm * cT + this->kalmanMatrix.R_state_process_noise_covariance_matrix;  // Innovation (or pre-fit residual) covariance

  Eigen::Matrix<scalar_t, 28, 1> sEy = s.lu().solve(ey);  // *Roughly* Optimal Kalman gain
  this->kalmanMatrix.xHat += pm * cT * sEy;  // Updated (a posteriori) state estimate

  Eigen::Matrix<scalar_t, 28, 18> sC = s.lu().solve(this->kalmanMatrix.C_matrix);
  this->kalmanMatrix.P_posteriori_covariance_matrix = (Eigen::Matrix<scalar_t, 18, 18>::Identity() - pm * cT * sC) * pm;  // *Roughly* Updated (a posteriori) estimate covariance

  Eigen::Matrix<scalar_t, 18, 18> pt = this->kalmanMatrix.P_posteriori_covariance_matrix.transpose();
  this->kalmanMatrix.P_posteriori_covariance_matrix = (this->kalmanMatrix.P_posteriori_covariance_matrix + pt) / 2.0;

  if (this->kalmanMatrix.P_posteriori_covariance_matrix.block(0, 0, 2, 2).determinant() > 0.000001) {
    this->kalmanMatrix.P_posteriori_covariance_matrix.block(0, 2, 2, 16).setZero();
    this->kalmanMatrix.P_posteriori_covariance_matrix.block(2, 0, 16, 2).setZero();
    this->kalmanMatrix.P_posteriori_covariance_matrix.block(0, 0, 2, 2) /= 10.;
  }
  ...
  this->lin_vel_filtered = 0.8*this->lin_vel_filtered + (1-0.8)*this->kalmanMatrix.xHat.segment<3>(3);

  this->rbdState.segment<3>(0) = this->kalmanMatrix.xHat.segment<3>(0);
